...
require __DIR__ . '/../csrf.php'; // Include CSRF class
...
?>
<section class="single-product space-top">
    <div class="container">
        <div class="row">
            <div class="col-md-6">
                <ol class="breadcrumb">
            ...
            <div class="col-md-7">
                <form action="/item?id=<?= htmlspecialchars($item[0]['id']) ?>" method="post">
                    <div class="single-product-details">
                        <h1 class="uppercase_"><?= htmlspecialchars($item[0]['title']) ?></h1>
                        <?php CSRF::csrfInputField() ?>
                        <input type="text" name="title" value="<?= htmlspecialchars($item[0]['title']) ?>" hidden>
                        <span class="product-price price_"><?= number_format($item[0]['price'], 2) ?> ฿ THB</span><br>
                        <span class="shipping-msg"><span class="hover-with-underline">Shipping</span> calculated at checkout.</span>
                        ...
                        <?php if ($inCart) : ?>
                            <button name="cart" type="submit" class="btn btn-main text-center button_addtocart" disabled>หยิบใส่ตะกร้า</button>
                        <?php else : ?>
                            <button name="cart" type="submit" class="btn btn-main text-center button_addtocart">หยิบใส่ตะกร้า</button>
                        <?php endif ?>
                    </div>
                </form>
                <a href="/cart"><button class="btn btn-main text-center button_gotocart">ไปที่ตะกร้า</button></a>
            </div>
        </div>
    </div>
</section>
